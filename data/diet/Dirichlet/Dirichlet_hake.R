...
ggsave(filename = "plots/diet/Dirichlet/Dirichlet_all_years.png", all_years[[2]], 
       bg = "transparent", width=150, height=160, units="mm", dpi=300)
...
to_ceattle <- function(df) {
  ...
  df3 <- data.frame(cbind(Pred = as.integer(rep(1, nrow(df2))),
                          ...
                          Stomach_proportion_by_weight = as.numeric(df2[, 7])))
  
  all_ages <- data.frame(Pred = rep(1, 75),
                         Prey = rep(1, 75),
                         Pred_sex = rep(0, 75),
                         Prey_sex = rep(0, 75),
                         Pred_age = rep(1:15, each = 5),
                         Prey_age = rep(1:5, times = 15),
                         Year = rep(0, 75),
                         Sample_size = rep(10, 75),
                         Stomach_proportion_by_weight = rep(0, 75))
